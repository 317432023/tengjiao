...
# 以下三个参数值由pom文件传递，由mvn中profiles的properties中提供并最终替换到此
baseZipName="${package-name}-${activeProfile}" #压缩包名称 publish-test.zip的publish
packageName="${package-name}" #命令启动包名 xx.jar的xx
mainclass="${boot-main}" #java -cp启动时，指定main入口类;命令：java -cp config;lib\*.jar;${packageName}.jar ${mainclass}
...
function start()
{
    ...
    then
        ...
        then
            if [ ${mainclass} ]
            then
                nohup java -cp config:lib\*.jar:${packageName}.jar ${mainclass} >${baseDirPath}/${packageName}.out 2>&1 &
                #nohup java -cp config:lib\*.jar:${packageName}.jar ${mainclass} >/dev/null 2>&1 &
            fi
        elif [ "${languageType}" == "java" ]
        then
            nohup java -jar ${packageName}.jar >/dev/null 2>&1 &
            # java -jar ${packageName}.jar
        elif [ "${languageType}" == "netcore" ]
        then
            #nohup dotnet run ${packageName} >/dev/null 2>&1 &
            nohup ${packageName} >/dev/null 2>&1 &
        fi
